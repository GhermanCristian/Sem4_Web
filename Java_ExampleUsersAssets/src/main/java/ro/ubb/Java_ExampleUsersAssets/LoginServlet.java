...
public class LoginServlet extends HttpServlet {
    private int getCount(String username, String password, Connection connection) throws SQLException {
        PreparedStatement checkCredentials = connection.prepareStatement("SELECT COUNT(*) FROM Users WHERE username = ? AND password = ?");
    ...
    private int getUserID(String username, String password){
        ...
        try(Connection connection = DBConnection.initializeDB()) {
            PreparedStatement checkCredentials = connection.prepareStatement("SELECT ID FROM Users WHERE username = ? AND password = ?");
